...
export const getSessionRepository = (dataSource: typeorm.DataSource) => {
  return dataSource.getRepository(SessionEntity).extend({
    /**
     *セッションの開始と関連データの更新
     *
     * @param {string} hash
    ...
    async startSession(
      ...
      if (id) {
        ...
        await this.createQueryBuilder()
          ...
          .execute();
        //セッションを抽出
        let result = await this.findOne({ where: { id, type } });
        if (result) {
          //セッションの有効時間を延期
          await this.createQueryBuilder()
            .update()
     ...
     * @memberof LocalDB
     */
    async createSession(type: string): Promise<string> {
      let id: string | null = uuid.v4();

      do {
        const result = await this.findOne({ where: { id: id!, type } });
        if (result) id = null;
      } while (id === null);
